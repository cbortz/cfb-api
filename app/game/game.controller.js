            ...
            try {
                ...
                let games = await db.any(`
                    SELECT
                        ...
                        hc.id AS home_conference_id,
                        hc.name AS home_conference,
                        hc.division AS home_division,
                        gt.points AS home_points,
                        gt.line_scores AS home_line_scores,
                        gt.win_prob AS home_post_win_prob,
                        gt.start_elo AS home_pregame_elo,
                        gt.end_elo AS home_postgame_elo,
                        away.id AS away_id,
                        away.school AS away_team,
                        ac.id AS away_conference_id,
                        ac.name AS away_conference,
                        ac.division AS away_division,
                        gt2.points AS away_points,
                        gt2.line_scores AS away_line_scores,
                        gt2.win_prob AS away_post_win_prob,
                        gt2.start_elo AS away_pregame_elo,
                        gt2.end_elo AS away_postgame_elo,
                        g.excitement AS excitement_index,
                        'https://www.youtube.com/watch?v=' || g.highlights AS highlights,
                        g.notes
                    FROM game g
